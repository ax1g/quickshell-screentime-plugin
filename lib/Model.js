...
// Yearly retro: cards derived only from month-scale aggregates, because raw
// day/app data covers just the 95-day retention window — an app-level or
// day-level "yearly" figure would silently undercount. Every stat here is
// true for the whole year. Returns { glyph, label, value, sub, color }.
function calendarTrivia(days, months, year) {
  ...
  var pctOfYear = function (ms) { return Math.round(ms / total * 100) }
  var out = []
  ...
  if (busiest) {
    out.push({
      glyph: "\uF0E7",
      label: "Peak month",
      value: busiest.label + " \u00b7 " + Math.round(busiest.ms / 3600000) + "h",
      sub: pctOfYear(busiest.ms) + "% of your year",
      color: "#ff6b6b"
    })
  }
  // 2 — Quietest month
  if (quietest && quietest !== busiest) {
    out.push({
      ...
      value: quietest.label + " \u00b7 " + Math.round(quietest.ms / 3600000) + "h",
      sub: pctOfYear(quietest.ms) + "% of your year",
      color: "#4ecdc4"
    })
  }
  // 3 — Year total
  var fullDays = Math.round(total / 86400000)
  out.push({
    glyph: "\uF017",
    label: "Year total",
    value: totalH + "h",
    sub: fullDays === 1 ? "about 1 full day" : "about " + fullDays + " full days",
    color: "#ffe66d"
  })
  // 4 — Month coverage, the honest look at how much of the year was tracked
  out.push({
    glyph: "\uF049",
    label: "Month coverage",
    value: coveredMonths + " of 12 months",
    sub: "months with any screen time",
    color: "#a78bfa"
  })
  // 5 — Vs last year when there is a previous year to compare against
  var prevTotal = yearTotal(days, months, year - 1)
  if (prevTotal > 0) {
    ...
    out.push({
      ...
      sub: delta >= 0 ? "more than " + (year - 1) + " \u2014 steady climb" : "less than " + (year - 1) + " \u2014 a lighter year",
      color: delta >= 0 ? "#fb923c" : "#34d399"
    })
  }
  return out
}
