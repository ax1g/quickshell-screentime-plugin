...
function firstDataYear(days, months) {
  var min = 0
  function scan(key) {
...
function monthlyTotals(days, months, year) {
  var out = []
  for (var m = 0; m < 12; m++) {
...
// `months` aggregates.
function yearTotal(days, months, year) {
  var total = 0
  for (var dk in days) {
    if (!Object.prototype.hasOwnProperty.call(days, dk)) continue
    var parts = String(dk).split("-")
...
// Yearly retro: light, month-scale trivia cards about a whole year. Each card
// is { glyph, label, value, sub, color } drawn from year/month aggregates, so
// historical years (where day-level data is pruned) stay honest. The one
// week-scale card (FULL-TIME FOCUS) needs raw days and only fires when the
// displayed year is the one being tracked day by day.
var YEAR_HOURS = 8760
var YEAR_HOURS_LEAP = 8784
var AWAKE_WEEK_HOURS = 112
var LOTR_EXTENDED_HOURS = 11.4
var WORK_WEEK_HOURS = 40
...
function calendarTrivia(days, months, year, todayKey) {
  var total = yearTotal(days, months, year)
  if (total <= 0) return []
  var mArr = monthlyTotals(days, months, year)
  var busiest = null
  var quietest = null
  for (var i = 0; i < mArr.length; i++) {
    if (mArr[i].ms <= 0) continue
    if (!busiest || mArr[i].ms > busiest.ms) busiest = mArr[i]
    if (!quietest || mArr[i].ms < quietest.ms) quietest = mArr[i]
  }
  var totalH = Math.round(total / 3600000)
  var yearMs = yearHours(year) * 3600000
  var out = []

  out.push({
    glyph: "\uF017",
    label: "SCREEN SHARE",
    value: totalH + "h on screens \u00b7 " + pctStr(total, yearMs) + " of " + year,
    sub: "The year's best-selling series. Greenlit for another season.",
    color: "#ffe66d"
  })

  var fullDays = Math.round(totalH / 24)
  out.push({
    glyph: "\uF0B1",
    label: "CHAIR TENURE",
    value: fullDays === 1
      ? "1 full day worth of screen time"
      : fullDays + " full 24h days worth of screen time",
    sub: "Your chair now lists 'star tenant' on its resume.",
    color: "#4ecdc4"
  })

  var lotrRuns = Math.max(1, Math.floor(totalH / LOTR_EXTENDED_HOURS))
  out.push({
    glyph: "\uF008",
    label: "MARATHON CREDENTIALS",
    value: totalH + "h = the extended LOTR trilogy \u00d7" + lotrRuns + " end to end",
    sub: "One does not simply log off after the credits.",
    color: "#fb923c"
  })
  ...
  if (busiest) {
    out.push({
      glyph: "\uF0E7",
      label: "POWER MONTH",
      value: busiest.label + " \u00b7 " + Math.round(busiest.ms / 3600000) + "h \u2014 your heaviest month",
      sub: "The other months are still intimidated.",
      color: "#ff6b6b"
    })
  }

  if (quietest && quietest !== busiest) {
    out.push({
      glyph: "\uF06C",
      label: "MONTH OF RESET",
      value: quietest.label + " \u00b7 " + Math.round(quietest.ms / 3600000) + "h \u2014 your most screen-free month",
      sub: "Even your router took a nap that month.",
      color: "#a78bfa"
    })
  }

  var workWeeks = Math.max(1, Math.round(totalH / WORK_WEEK_HOURS))
  out.push({
    glyph: "\uF2F1",
    label: "MOONLIGHT JOB",
    value: workWeeks === 1
      ? "1 full 40-hour work week of focus"
      : workWeeks + " full 40-hour work weeks of focus",
    sub: "You've been moonlighting as a monitor.",
    color: "#34d399"
  })

  // Week-scale card: only truthful for the year that still has raw day data.
  var thisYear = todayKey ? Number(String(todayKey).split("-")[0]) : new Date().getFullYear()
  if (Number(year) === thisYear) {
    var weekMs = weekTotal(weekTrend(days, todayKey))
    if (weekMs > 0) {
      var weekH = Math.round(weekMs / 3600000)
      out.push({
        glyph: "\uF0F3",
        label: "FULL-TIME FOCUS",
        value: pctStr(weekMs, AWAKE_WEEK_HOURS * 3600000)
          + " of your awake week on screens (" + weekH + "h / " + AWAKE_WEEK_HOURS + "h)",
        sub: "Your wake schedule issued a formal thank-you card.",
        color: "#f472b6"
      })
    }
  }
  ...
  out.push({
    glyph: "\uF2D0",
    label: "PIXEL PERSONALITY",
    value: pctStr(total, yearMs) + " of this year spent with glowing rectangles",
    sub: "You run on luminance now. Glow responsibly \u2014 but glamorously.",
    color: "#60a5fa"
  })
