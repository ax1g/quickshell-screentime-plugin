...
// rows; missing data shows "—" placeholders. weekEndKey anchors the
...
// `days`, `months` lumps, per-day `years` archive). The no-overlap rule —
// pruned days live only in the archive, never in the month lumps — is
// enforced here, once, instead of once per reader. Returns
...
function yearView(days, months, years, year, todayKey, accentHex) {
  var summary = yearSummary(days, months, years, year, todayKey)
  var facts = yearFacts(days, months, years, year, todayKey, accentHex)
  return {
