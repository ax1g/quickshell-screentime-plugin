...
function monthlyTotals(days, months, year, years) {
  ...
  for (var m = 0; m < 12; m++) {
    ...
    var total = numMs(months && months[monthKey])
    ...
    for (var dk in days) {
      ...
      if (Number(parts[0]) !== y) continue
      if (Number(parts[1]) - 1 !== m) continue
      var d = days[dk]
      total += d ? numMs(d.total) : 0
    }
...
function yearTotal(days, months, year, years) {
  ...
  for (var mk in months) {
    ...
    if (Number(mParts[0]) !== y) continue
    total += numMs(months[mk])
  }
...
function yearDayTotals(years, days, year, todayKey) {
  ...
  for (var m = 0; m < 12; m++) {
    ...
    for (var d = 1; d <= dim; d++) {
      ...
      var ms = numMs(arch[key])
      var raw = days && days[key]
      if (raw) ms += numMs(raw.total)
