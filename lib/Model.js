...
// The year archive maps "YYYY" to { "YYYY-MM-DD": ms }. Rebuilds it so only
// plain year objects holding numeric day totals survive.
function sanitizeYears(years) {
  if (!isPlainObject(years)) return {}
  var out = {}
  for (var yk in years) {
    if (!Object.prototype.hasOwnProperty.call(years, yk)) continue
    if (!isPlainObject(years[yk])) continue
    var day = {}
  ...
  if (dayTotals.length > 0) {
    ...
    if (active > 0) {
      out.push({
        ...
        value: fmt(Math.round(total / active)) + " per active day",
        sub: "A solid daily shift, no overtime attitude.",
        color: "#34d399"
      })

      var wd = weekdayPattern(dayTotals, total)
      out.push({
        glyph: "\uF073",
